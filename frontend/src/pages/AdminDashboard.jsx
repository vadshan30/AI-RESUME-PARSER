...
import { Users, FileText, Activity, Star, Loader2 } from 'lucide-react';
...
const AdminDashboard = () => {
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-slate-900 flex items-center justify-center text-rose-400 font-bold text-xl">
        {error}
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-slate-900 text-slate-100 font-sans pb-20">
      <header className="bg-slate-800/50 border-b border-slate-700 backdrop-blur-md sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <div className="text-sm text-slate-400">System Dashboard</div>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-6 shadow-lg flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-white mt-1">{data?.total_users}</p>
            </div>
            <div className="bg-indigo-500/20 p-3 rounded-lg"><Users className="w-6 h-6 text-indigo-400" /></div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          {/* Recent Resumes */}
          <div className="bg-slate-800/30 border border-slate-700 rounded-2xl p-6 shadow-xl">
            <h2 className="text-lg font-bold text-white mb-6">Recent Scans</h2>
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm text-slate-400">
                <thead className="text-xs uppercase bg-slate-800/50 text-slate-300">
                  <tr>
                ...
                <tbody>
                  {resumes.map(r => (
                    <tr key={r.id} className="border-b border-slate-700/50 last:border-0 hover:bg-slate-700/20">
                      <td className="px-4 py-3 text-white font-medium truncate max-w-[150px]">{r.filename}</td>
