...
for (const [categoryName, data] of Object.entries(categories)) {
  for (let i = 0; i < data.count; i++) {
    ...
    // Skip the ones that match our manual examples so we don't duplicate
    if (title === "Senior React Developer" || title === "Senior Java Developer" || title === "AI Engineer" || title === "Senior DevOps Engineer" || title === "Junior Full Stack Developer") {
      if(i === 0) continue; // simplistic deduplication
    }
    ...
    const resume = {
      ...
      title: title,
      role: categoryName.split(' ')[0], // Simplistic role naming
      experience: exp,
      ...
      education: [
        {
          ...
          cgpa: Number((Math.random() * 1.5 + 2.5).toFixed(1)),
...
const allResumes = [...manualExamples, ...generatedResumes];

// Output file format
const fileContent = `export interface MockResume {
  id: string;
  name: string;
  title: string;
  role: string;
...
export const MOCK_RESUMES: MockResume[] = ${JSON.stringify(allResumes, null, 2)};
`;

const outPath = path.join(__dirname, 'frontend', 'src', 'data', 'MockResumes.ts');
fs.mkdirSync(path.dirname(outPath), { recursive: true });
fs.writeFileSync(outPath, fileContent);
console.log(`Generated ${allResumes.length} resumes at ${outPath}`);
